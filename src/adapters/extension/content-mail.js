 ...
 * と、退路（Gmail の常設リンク）の見せ方だけを持つ。
 */

/**
 * URL を**新しいタブで**開く。**anchor を作って click する**（window.open は使わない）。
 *
 ...
 *
 * **`target="_blank"` は常に付ける。** 付けないと、`mailto:` に web ハンドラ
 * （Gmail 等）を登録している利用者では現在のタブが遷移し、見ていた書籍ページが
 * 消える（実機での指摘）。以前は「開けたか」を推定するために target を外して
 * いたが、推定ごと捨てた（→ core の mailopen.js と SPEC「メールの開き方」）。
 *
 * @param {object} args
 ...
 */
function jimotoOpenMailUrl({ url, root }) {
  const link = jimotoEl('a', {
    href: url,
    target: '_blank',
    rel: 'noopener',
    style: 'display: none',
 ...
 *   fallback は Gmail の常設リンク。content.js がパネル木に挿す
 */
function jimotoMakeMailActions({ core, profile, getArgs, ui, root }) {
  ...
  /**
   * 常設リンクの出し入れ。**`gmail` に設定済みなら出さない。**
   * 既に Gmail で開くのだから「Gmail で開く」を並べる意味が無く、
   * 押しても同じことが起きるだけの飾りになる。
   */
  const syncFallback = () =>
    fallback.classList.toggle('jimoto-fallback-hidden', mailOpener === 'gmail');
   ...
   * 退路は常設なので、前のクリックからいくらでも間が空きうる（その間に財源も
   * 冊数も変わる）。古い plan を開き直すと、利用者には「同じ下書きを別の方法で
   * 開くボタン」に見えたまま旧課題番号の下書きが飛ぶ（誤発注は執行事故になる）。
   *
   ...
   *
   * **設定を書き換える根拠は利用者のクリックだけ。** 推定で黙って切り替えると、
   * 外れたときに mailto へ戻す道が設定画面しか無くなる（推定は 2026-09-05 に
   * 捨てたが、この原則は残す）。
   */
  const rememberGmailChoice = async () => {
    ...
    try {
      ...
      jimotoOpenMailUrl({ url: target.url, root });
    } catch (error) {
  ...
  // 退路。**常設**にする。ハンドラの登録状況は API から照会できず、推定は
  // 新しいタブで開く以上どうやっても外れる（→ core の mailopen.js）。
  // 出し分けを諦める代わりに、退路そのものは最初から見えている。
  // エラーではなく案内なので赤は使わない（→ content.css の .jimoto-fallback）
  const fallback = jimotoEl('div', { class: 'jimoto-fallback' }, [
    jimotoEl('div', { text: 'メーラーが開かない場合は' }),
  ...
  const openMail = async () => {
    ...
    // ここで「開けたか」は見ない。新しいタブで開く以上、自分が作ったタブでも
    // visibility が変わるので推定は必ず外れる。退路は常設で下に出ている
  };
