...
- **例示データに実在の人名・所属・居室を書かない。** placeholder・コメントの例・
  ...
  | 用途 | 値 |
  | --- | --- |
  | UI の placeholder（人名） | `山田 太郎` / かな `やまだ たろう` |
  | テスト fixture（人名） | `テスト 太郎` / かな `てすと たろう` / メール `taro@example.ac.jp` |
  | 所属 | `○○大学 △△学部 ××研究室` |
  | 配達場所 | `△△棟 3F 305号室` |
  | 電話 | `03-0000-0000` |

  人名を UI とテストで二本立てにしているのは、`compose.js` の簡略版が署名に
  依頼者名を入れるため、書誌の著者名（`山田 太郎`）と同じ値を fixture に使うと
  「簡略版は著者を落とす」テストが必ず衝突するから。fixture 側は既存の
  `テスト書名` / `テスト著者` の語彙に揃える。
